  ...
  app.innerHTML = `
    ...
    <ul>
      ${students.map((student, index) => `
        <li>
          ${student.name} - 평균 점수: ${calculateAveragePercentile(student)}%
...
function renderEvaluationPage() {
  app.innerHTML = `
    ...
    <input type="text" id="stageName" placeholder="단계 이름 입력" value="${stageNames[currentStage - 1]}">
...
function saveEvaluation() {
  ...
  stageNames[currentStage - 1] = stageNameInput.value;

  // Save current stage scores
...
function showFinalEvaluation(index) {
    ...
    let summary = `<h3>${student.name}[실습] 경복궁 문화유산해설사 1회차 실습평가 피드백:</h3>`;
    summary += `<ul>`;
    
    Object.keys(student.scores).forEach(stage => {
      summary += `<li><strong>장소 ${stage.replace('stage', '')} (${stageNames[Number(stage.replace('stage', ''))]}):</strong><ul>`;
  ...
  function getScoreMessage(percentile) {
    if (percentile >= 100) {
      ...
      return "잘 해낼 수 있지만 디테일적인 부분을 더 깊에 설명 할 필요가 있음";
